...
const techOptions = ['JAVA', 'Python', 'JavaScript', 'Spring', 'React', 'Node.js', 'Vue', 'Angular', 'Django', 'Kotlin'];
const positions = ref([{ role: '', count: 1 }]);
const roleOptions = ['프론트엔드', '백엔드', '디자이너', 'PM', '기획자', '데브옵스', '안드로이드 개발자', 'IOS 개발자', '크로스 플랫폼 개발자'];

const addSkill = () => {
  if (selectedSkill.value && !selectedSkills.value.includes(selectedSkill.value)) {
